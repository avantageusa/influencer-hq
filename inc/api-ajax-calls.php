...
// Map from front-end level keys (lowercase) to API referralLevel values (uppercase).
define( 'EQUITY_LEVEL_MAP', array(
    'l1'   => 'L1',
    'l2'   => 'L2',
    'l3'   => 'L3',
    'kick' => 'KICK',
...
// Equity Chart Data
// ---------------------------------------------------------------------------
add_action( 'wp_ajax_equity_chart_data',        'equity_chart_data_ajax' );
add_action( 'wp_ajax_nopriv_equity_chart_data', 'equity_chart_data_ajax' );

function equity_chart_data_ajax() {

    if ( ! check_ajax_referer( 'equity_chart_nonce', 'nonce', false ) ) {
        wp_send_json_error( 'Security check failed.', 403 );
    }

    // --- sanitise & validate inputs ---
    $allowed_periods = array( 'week', 'month', 'year', 'all' );
    $period = sanitize_text_field( $_POST['period'] ?? 'week' );
    if ( ! in_array( $period, $allowed_periods, true ) ) {
        wp_send_json_error( 'Invalid period.' );
    }

    $allowed_levels = array_merge( array_keys( EQUITY_LEVEL_MAP ), array( 'all' ) );
    $raw_levels     = explode( ',', sanitize_text_field( $_POST['levels'] ?? 'all' ) );
    $levels         = array_values(
        array_filter( $raw_levels, fn( $l ) => in_array( $l, $allowed_levels, true ) )
    );
    if ( empty( $levels ) ) {
        $levels = array( 'all' );
    }

    // Expand 'all' and map to API-level names e.g. ['L1','L2','KICK']
    if ( in_array( 'all', $levels, true ) ) {
        $api_levels = array_values( EQUITY_LEVEL_MAP );
    } else {
        $api_levels = array_values( array_map(
            fn( $l ) => EQUITY_LEVEL_MAP[ $l ],
            array_filter( $levels, fn( $l ) => isset( EQUITY_LEVEL_MAP[ $l ] ) )
        ) );
    }

    $wp_user_id = get_current_user_id();
    $wpu_id     = 'influencerhq-wpu-' . $wp_user_id;
    ...
    $api_url = INFLUENCER_API_BASE
        . '/referral/user/' . rawurlencode( $wpu_id )
        . '/equity/totals'
        . '?' . http_build_query( array(
            'referralLevel' => implode( ',', $api_levels ),
            'period'        => $period,
        ) );
    ...
    $debug = array(
        ...
        'method'      => 'GET',
        'period'      => $period,
        'levels_sent' => $api_levels,
        'http_status' => null,
        'headers'     => array(
            'Authorization' => 'Bearer ' . substr( $id_token, 0, 40 ) . '…(truncated)',
        ),
    );
    ...
    if ( is_wp_error( $response ) ) {
        $debug['http_status'] = 0;
        wp_send_json_error( array( 'message' => $response->get_error_message(), '_debug' => $debug ) );
        return;
    }
    ...
    $api_data = ( isset( $body['data'] ) && is_array( $body['data'] ) ) ? $body['data'] : $body;

    // Ensure datasets carry Chart.js styling (API drives the data; we supply colours).
    $level_styles = array(
        'L1'   => array( 'borderColor' => '#D4AF37', 'backgroundColor' => '#D4AF3726' ),
        'L2'   => array( 'borderColor' => '#8B7536', 'backgroundColor' => '#8B753626' ),
        'L3'   => array( 'borderColor' => '#C0932F', 'backgroundColor' => '#C0932F26' ),
        'KICK' => array( 'borderColor' => '#53FC18', 'backgroundColor' => '#53FC1826' ),
        'LIVE' => array( 'borderColor' => '#A78BFA', 'backgroundColor' => '#A78BFA26' ),
    );

    if ( ! empty( $api_data['datasets'] ) && is_array( $api_data['datasets'] ) ) {
        foreach ( $api_data['datasets'] as &$ds ) {
            $lvl    = strtoupper( $ds['level'] ?? '' );
            $styles = $level_styles[ $lvl ] ?? array();
            $ds     = array_merge( array(
                'tension'          => 0.4,
                'fill'             => false,
                'pointRadius'      => 3,
                'pointHoverRadius' => 5,
    ...
    $api_data['_debug'] = $debug;
    wp_send_json_success( $api_data );
}

// ---------------------------------------------------------------------------
// SSO Debug — fire /account/oauth/start-session for the current user and
// return the raw HTTP status + body so the equity page can display it.
// Only available to logged-in users.
// ---------------------------------------------------------------------------
add_action( 'wp_ajax_equity_sso_debug', 'equity_sso_debug_ajax' );

function equity_sso_debug_ajax() {

    if ( ! check_ajax_referer( 'equity_sso_debug_nonce', 'nonce', false ) ) {
        wp_send_json_error( 'Security check failed.', 403 );
    }
    ...
    $api_url = INFLUENCER_API_BASE . '/account/oauth/start-session';

    $payload = array(
        'oauthLoginType' => 'InfluencerHq',
        'payload'        => array(
            'id'        => 'influencerhq-wpu-' . $user_id,
    ...
    if ( is_wp_error( $response ) ) {
        ...
        $result['raw_body']    = $response->get_error_message();
        $result['stored']      = false;
    ...
    $result['raw_body']    = $body ?? $raw_body; // decoded if possible
    $result['stored']      = false;

    // Try every plausible response envelope and store whatever tokens we find.
    $tokens = null;
    if ( ! empty( $body['data']['AuthenticationResult'] ) ) {
        $tokens = $body['data']['AuthenticationResult'];
    } elseif ( ! empty( $body['data'] ) && ! empty( $body['data']['AccessToken'] ) ) {
        $tokens = $body['data'];
    } elseif ( ! empty( $body['AuthenticationResult'] ) ) {
        $tokens = $body['AuthenticationResult'];
    } elseif ( ! empty( $body['AccessToken'] ) ) {
        $tokens = $body;
    }

    if ( $tokens && ! empty( $tokens['AccessToken'] ) ) {
        update_user_meta( $user_id, 'ihq_access_token',  $tokens['AccessToken'] );
        update_user_meta( $user_id, 'ihq_id_token',      $tokens['IdToken']      ?? '' );
        update_user_meta( $user_id, 'ihq_refresh_token', $tokens['RefreshToken'] ?? '' );
        update_user_meta( $user_id, 'ihq_token_type',    $tokens['TokenType']    ?? 'Bearer' );
        update_user_meta( $user_id, 'ihq_token_expires', time() + (int) ( $tokens['ExpiresIn'] ?? 3600 ) );
        $result['stored'] = true;
    }
